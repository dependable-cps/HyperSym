...
from src.parser import check_Observational_Determinism
import numpy as np
import random
from z3 import *
import time
...
def Observational_Determinism_Verifi(Formula,Num,dts_file_location,method):
        ...
        #T = 15
        check_Observational_Determinism(Formula)
        # Part to remove
        part_to_remove = "E_V1 F_V2 . A_P . "

        # Remove the specified part
        cleaned_formula = Formula.replace(part_to_remove, "")
        ...
        matches = re.findall(pattern, cleaned_formula)
        ...
        original_formula=And(
                ...
                + [l1[0][labelToNum['Ss']]]
                + [Implies(
                    And([l1[0][labelToNum['Ss']] == l2[0][labelToNum['Ss']]]),
                    And([l1[t][labelToNum['I']] == l2[t][labelToNum['I']] for t in range(T)])
                )]

                + [Not([Or([l1[t][labelToNum['I']] for t in range(Times[1])])])]
                + [Not([Or([l2[t][labelToNum['I']] for t in range(Times[1])])])]
                + [Not([Or([l1[t][labelToNum['G']] for t in range(T)])])]  # Finally Goal state in path2
                + [Not([Or([l2[t][labelToNum['G']] for t in range(T)])])]
                + [Not([l1[T-1][labelToNum['G']] == l2[T-1][labelToNum['G']]])] # goal state should be same at the end
                + [Not([Exists(
        ...
        end1 = time.time()
        negated_formula = original_formula
        solver.add(negated_formula)
        start = time.time()
        res = solver.check()
        end = time.time()
        print("Observational Determinism solved in " + str(end - start) + " sec")
