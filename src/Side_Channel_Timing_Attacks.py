...
def Side_Channel_Timing_Attacks_Synthesis(Formula,Num,dts_file_location,method):
    ...
    And(t_charging_1 - 2 <= t_charging_2, t_charging_2 <= t_charging_1 + 2)"""
        ...
        print(" Side Channel Timing solved in " + str(end - start) + " sec")
        strategy = []
        strategy1 = []
        if res == sat:
            m = solver.model()
        ...
        cdts.plot([], [], 'Side_Channel_Timing_Attacks_Problem')
        cdts.plot(strategy,strategy1, 'Side_Channel_Timing_Attacks_solution')
        solver.pop()
