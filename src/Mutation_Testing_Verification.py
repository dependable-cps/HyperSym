...
import random
from z3 import *
from src.parser import check_Mutation_Testing
import time
import re
...
def Mutation_Testing_Verifi(Formula,Num,dts_file_location,method):
        start1 = time.time()
        ...
        check_Mutation_Testing(Formula)

        part_to_remove = "E_V1 F_V2 . [H^1 t_V1 & H^1 t_V2]^[0,10] & "

        # Remove the specified part
        modified_formula = Formula.replace(part_to_remove, "")
        ...
        negated_formula = And(
            ...
            # Negating other constraints
            Not(Or([Or([l1[t][labelToNum['I']] for t in range(Times[1])])])),
            # Negating initial 'I' state constraint for path1
            Not(Or([Or([l2[t][labelToNum['I']] for t in range(Times[1])])])),
            # Negating initial 'I' state constraint for path2
            Not(Or([Or([l1[t][labelToNum['G']] for t in range(T)])])),  # Negating Goal state constraint for path1
            Not(Or([Or([l2[t][labelToNum['G']] for t in range(T)])])),  # Negating Goal state constraint for path2
            Not(Exists(
        ...
        """
        negated_formula=And(
                [(S1[t] >= 0) for t in range(T)] +
                [(S1[t] < n) for t in range(T)] +  # Legal states in path1
                ...
                [(S2[t] >= 0) for t in range(T)] +
                [(S2[t] < n) for t in range(T)] +  # Legal states in path2
    ...
    # Ensure paths diverge based on mutation (mutated traces go to different charging states)
    Or(
        [And(l1[T - 1][labelToNum['G']], S1[T - 1] == stateToNum['C1'])],  # Path1 reaches charging state C1
        [And(l1[T - 1][labelToNum['G']], S1[T - 1] == stateToNum['C2'])],  # Path1 reaches charging state C2
        [And(l2[T - 1][labelToNum['G']], S2[T - 1] == stateToNum['C1'])],  # Path2 reaches charging state C1
        [And(l2[T - 1][labelToNum['G']], S2[T - 1] == stateToNum['C2'])]   # Path2 reaches charging state C2
    )
"""
        end1 = time.time()
        ...
        print("Mutation Testing solved in " + str(end - start) + " sec")
