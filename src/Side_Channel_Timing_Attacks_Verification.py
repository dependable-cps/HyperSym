...
import random
from z3 import *
from src.parser import check_Side_Channel_Timing_Attacks
import time
import re
...
def Side_Channel_Timing_Attacks_Verifi(Formula,Num,dts_file_location,method):
        start1 = time.time()
        ...
        check_Side_Channel_Timing_Attacks(Formula)

        # Part to remove
        part_to_remove = "F_V1 F_V2 . A_P E_P ."
        ...
        solver.add(
            And(
                ...
                + [l2[0][labelToNum['Ss']]]
                + [l1[0][labelToNum['Ss']]]
                + [And([Not(l1[t][labelToNum['I']]) for t in range(Times[1])])]  # Negate label 'I' in path1
                + [And([Not(l2[t][labelToNum['I']]) for t in range(Times[1])])]  # Negate label 'I' in path2
                + [And([Not(l1[t][labelToNum['G']]) for t in range(T)])]  # Negate label 'G' in path1
                + [And([Not(l2[t][labelToNum['G']]) for t in range(T)])]  # Negate label 'G' in path2
                + [Not(Exists(
        ...
        end1 = time.time()
        """    # Side-channel timing attack constraints
    # Ensure mission execution times are close enough
    And(t1 - 2 <= t2, t2 <= t1 + 2),
    # Ensure both paths end up in the charging state within close enough time
    And(t_charging_1 - 2 <= t_charging_2, t_charging_2 <= t_charging_1 + 2)"""
        start = time.time()
        res = solver.check()
        end = time.time()
        print("Side Channel Timing Attacks solved in " + str(end - start) + " sec")
