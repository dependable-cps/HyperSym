...
def Initial_s_o_verifi(Formula,Num,dts_file_location,method):
        ...
        original_formula=And(
                ...
                + [l1[t][labelToNum[l]] == l2[t][labelToNum[l]] for t in range(T) for l in
                   ['D', 'U', 'L', 'R']]  # Action labels are same
        ...
        negated_formula = Not(original_formula)
        ...
        print("verifivation result : "+str(res))
        strategy = []
        strategy1 = []
        if res == sat:
            m = solver.model()
