...
def Initial_s_o_v(Formula,Num,dts_file_location,method):
        ...
        solver.add(
            And(
                ...
                [Not(S1[0] != S2[0])] +  # Paths start from different states
                [Not(l1[t][labelToNum[l]] == l2[t][labelToNum[l]]) for t in range(T) for l in
                 ['D', 'U', 'L', 'R']] +  # Action labels are the same
                [Not(Or([l1[t][labelToNum['I']] for t in range(Times[1])]))] +
                [Not(Or([l2[t][labelToNum['I']] for t in range(Times[1])]))] +
                [Not(Or([l1[t][labelToNum['G']] for t in range(T)]))] +  # Finally Goal state in path1
                [Not(Or([l2[t][labelToNum['G']] for t in range(T)]))] +  # Finally Goal state in path2
                [Not(Implies(l1[t][labelToNum['G']], l2[t][labelToNum['G']])) for t in range(T)] +
                [Not(Implies(l2[t][labelToNum['G']], l1[t][labelToNum['G']])) for t in range(T)]
            )
        )
        end1 = time.time()
